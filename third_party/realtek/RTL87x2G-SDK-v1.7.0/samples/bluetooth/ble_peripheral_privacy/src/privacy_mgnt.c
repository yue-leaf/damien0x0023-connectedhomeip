...
T_PRIVACY_STATE privacy_handle_resolv_list_int(void)
{
    ...
    for (i = 0; i < bond_storage_num; i++)
    {
        if (privacy_table[i].is_used && privacy_table[i].state != PRIVACY_RESOLVING_LIST_IDLE)
        {
            ...
            {
                T_LE_KEY_ENTRY *p_entry = le_find_key_entry_by_idx(i);
                uint8_t remote_irk[24];

                if (le_get_dev_irk(p_entry, true, remote_irk))
                {
                    APP_PRINT_INFO4("[PRIVACY] Add: i %d, BD %s, type %d, remote_irk %s", i,
                                    TRACE_BDADDR(privacy_table[i].addr),
                                    privacy_table[i].remote_bd_type,
                                    TRACE_BINARY(24, remote_irk));

                    bool use_local_irk = false;

                    if (p_entry->flags & LE_KEY_STORE_LOCAL_LTK_BIT)
                    {
                        use_local_irk = true;
                    }
                    cause = le_privacy_add_resolv_list(privacy_table[i].remote_bd_type,
                                                       privacy_table[i].addr,
                                                       remote_irk, use_local_irk);
                }
                else
                {
                    APP_PRINT_ERROR1("[PRIVACY] Add: i %d, Find no remote irk", i);
                    cause = GAP_CAUSE_NOT_FIND_IRK;
                }
