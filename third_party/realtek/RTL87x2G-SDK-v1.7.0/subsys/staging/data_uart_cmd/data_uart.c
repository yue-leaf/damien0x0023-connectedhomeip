...
{
    UART_SendData(UART4, (uint8_t *)&ch, 1);
    /* wait tx fifo empty */
    while (UART_GetFlagState(UART4, UART_FLAG_THR_TSR_EMPTY) != SET);
...
void data_uart_print(char *fmt, ...)
{
    ...
    va_list  ap;
    va_start(ap, fmt);
    (void)data_uart_vsprintf(0, fmt, ((const int *)&fmt) + 3);
    va_end(ap);
...
void UART4_Handler(void)
{
    ...
    /* read interrupt id */
    interrupt_id = UART_GetIID(UART4);

    /* disable interrupt */
    UART_INTConfig(UART4, UART_INT_RD_AVA | UART_INT_LINE_STS, DISABLE);
    ...
    case UART_INT_ID_RX_LEVEL_REACH:
        UART_ReceiveData(UART4, &rx_char, 1);
    ...
    UART_INTConfig(UART4, UART_INT_RD_AVA, ENABLE);

    return;
}

/**
 * @brief  Initializes the Data UART4.
 *
 ...
 * \code{.c}
    ...
    void app_handle_io_msg(T_IO_MSG io_msg)
    {
        ...
        case IO_MSG_TYPE_UART:
            // We handle user command informations from Data UART4 in this branch.
...
//    Pinmux_Deinit(DATA_UART_TX_PIN);
//    Pinmux_Deinit(DATA_UART_RX_PIN);
    h_event_q = event_queue_handle;
    h_io_q = io_queue_handle;

    RCC_PeriphClockCmd(APBPeriph_UART4, APBPeriph_UART4_CLOCK, ENABLE);

    Pinmux_Config(DATA_UART_TX_PIN, UART4_TX);
    Pinmux_Config(DATA_UART_RX_PIN, UART4_RX);
    ...
    UART_Init(UART4, &uartInitStruct);
    UART_INTConfig(UART4, UART_INT_RD_AVA, ENABLE);

    /*  Enable UART4 IRQ  */
    NVIC_InitTypeDef nvic_init_struct;
    nvic_init_struct.NVIC_IRQChannel         = UART4_IRQn;
